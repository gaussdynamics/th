...
Longitudinal train dynamics (LTD) concerns the motion and internal force distribution along the length of a train. A freight consist is a distributed mechanical system of tens to hundreds of coupled masses whose interactions with one another and with the track profile are strongly nonlinear \citep{Cole2006HandbookLTD}. Simulating those interactions faithfully requires solving differential equations for every vehicle in the chain, at a cost that spans roughly three orders of magnitude across the fidelity ladder: reduced or lumped-parameter models run in fractions of a CPU-hour \citep{perez2016reduced}, whole-trip high-fidelity LTD in tens of CPU-hours \citep{wu2019highfidelity}, and full multibody vehicle dynamics higher still.

This computational cost is the central obstacle. Control design and operational planning are not single-evaluation activities; they require the model to be evaluated hundreds or thousands of times within an optimization loop. A surrogate model addresses this by learning an approximate input-to-response mapping that evaluates in milliseconds while retaining the underlying physical structure. Surrogates complement rather than replace deterministic physics models: the high-fidelity simulator generates the training data and supplies the physical grounding, and the surrogate supplies the speed.
...
Data-driven rail modeling falls into three overlapping categories. The first embeds learned components within classical multibody or finite-element frameworks: surrogate modules trained on finite-element outputs have approximated nonlinear mechanical behavior within vehicle-track simulation at significant runtime reduction \citep{nie_data_driven_dynamics}, and more recent work packages a neural replacement for a full multibody model through standardized co-simulation interfaces, demonstrating speedups of several orders of magnitude \citep{zhou_surrogate_integration}. The second category targets in-train forces, which are difficult to measure directly in service: supervised approaches map telemetry to coupler force estimates, with causally structured convolutional networks reproducing simulator-derived force histories accurately \citep{zhang_in_train_forces}, and extensions add multi-task and transfer learning to generalize across couplers and configurations \citep{intrainnet}. The third category operates above the dynamics entirely, approximating the evaluation of candidate schedules within an optimization loop \citep{liu_rescheduling_surrogate}.

Three limitations recur across this body of work and shape the present study. First, generalization across consist configurations remains a persistent challenge: force-estimation models are often trained on narrow scenario subsets, and even with transfer learning, performance degrades where strong nonlinear vehicle-to-vehicle interaction dominates \citep{zhang_in_train_forces,intrainnet}. Second, reliance on simulated labels means that simulator-to-reality discrepancy compounds through a purely data-driven mapping. Third, hybrid approaches that retain physical structure generally aim to replicate nominal simulator output rather than to achieve robustness under distribution shift. In nearly every case, the training data is generated by a tool that the authors do not describe in reproducible detail, which is the specific gap this paper addresses.
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.60\linewidth]{figures/ltd_stage03_slack_coupler_linear_vs_nonlinear_force_delta_speeddiff.png}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.60\linewidth]{figures/ltd_stage05_N_vehicle_coupler_force_heatmap.png}
...
Grade and curvature are no longer evaluated at a single global position. Each vehicle samples $\sin\theta(x_i)$ and, optionally, $\kappa(x_i)$ at its own location, so that $G_i = m_i g \sin\theta(x_i)$. This introduces spatial dispersion along the train, since the head and tail may occupy different grades simultaneously, exciting relative motion and coupler cycling even where the speed traces remain nearly overlaid. For a consist more than a kilometer long this is not a second-order effect, and it is the point at which route data quality begins to bear directly on the dynamics.

\subsection{Stage 7: Actuator Dynamics and Power-Limited Traction}

The state is augmented by first-order lag variables for the commanded brake and traction levels on each vehicle, driven by exogenous commands:
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:powercap}
\end{equation}
with $P_{\max}$ an illustrative constant power ceiling and $\varepsilon$ a small speed regularizer. Figure~\ref{fig:stage07} shows lead speed together with the lag states for a scenario in which traction and braking commands overlap; the plotted states are commands, not achieved forces, which additionally reflect Equation~\ref{eq:powercap} and brake saturation. This stage is what makes the control plan $\mathcal{U}$ physically meaningful rather than instantaneous, and it is a precondition for the control application the surrogate is intended to serve.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.60\linewidth]{figures/ltd_stage07_brake_buildup_power_limited_traction_states.png}
...
\begin{equation}
  ...
  \label{eq:curvature}
\end{equation}
clamped to a minimum turning radius. Speed limit is converted from the map's tagged value to meters per second where available, falling back to an estimate based on Federal Railroad Administration track class where OSM coverage is missing \citep{fra_track_classes}.
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.60\linewidth]{figures/route_grade_curvature.png}
  \caption{The three derived fields the simulator consumes, plotted against chainage: grade as a percentage (top), the curvature proxy (lower left), and the step-valued speed limit (lower right). The noisier grade over the first 20~km corresponds to the urban approach.}\label{fig:profile}
\end{figure}
