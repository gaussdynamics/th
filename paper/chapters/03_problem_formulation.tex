...
The surrogate model built later in this thesis does not try to learn $\mathcal{F}_n$ from a blank slate.
Instead, it starts from this known physics right-hand side and learns a correction on top of it, so that the predicted rate of change takes the form $\mathcal{F}_n + \Delta\mathcal{F}_\theta$, where $\Delta\mathcal{F}_\theta$ is a small learned term produced by a graph neural network.
Building the physics into the model this way, rather than treating it as just another input feature or as a soft penalty added to the loss, gives the surrogate a strong head start and keeps its behavior anchored to something physically sensible even for consists and routes it has not seen during training.
...
The surrogate introduced later in this thesis approximates $F$ using a message-passing graph neural network that operates directly on the node and edge tensors defined above, learning a correction to the known physics rather than replacing it (\cref{sec:tensor_dynamics_form}).
...
\begin{equation}
    ...
    \right\}_{k=1}^{M}.
\end{equation}
The surrogate $f_\theta$ maps structured inputs to trajectory and summary predictions in the same tensor layout, predicting a correction on top of the known physics right-hand side (\cref{sec:tensor_dynamics_form}) rather than the dynamics outright.
...
\end{equation}
with rollout losses on node and edge tensors, summary losses, a quantile loss for the safety-relevant margin, and an optional physics-consistency penalty.
...
\end{equation}
and $\mathcal{L}_{Q}$ aggregates $\rho_p$ over selected quantile levels. In this thesis it is applied narrowly, to the handful of safety-relevant quantities, chiefly coupler force, consistent with the narrower role of uncertainty described in \cref{sec:uncertainty_sources}, rather than to every predicted channel.

\textbf{Physics-consistency penalty.} $\mathcal{L}_{P}$ is a small, optional penalty on top of the residual architecture described in \cref{sec:tensor_dynamics_form}.
Because the surrogate already predicts a correction to a known physics right-hand side rather than the full dynamics from scratch, most of the physical plausibility is built into the model's structure and does not need to be enforced after the fact.
$\mathcal{L}_P$ only needs to catch smaller inconsistencies, for example a coupler-force channel that does not agree with the accelerations of the two vehicles it connects.

\textbf{Remark.} A later chapter instantiates these terms for the specific graph neural network architecture and training regime used in this thesis.
...
The chapter after that introduces the surrogate model itself: a graph neural network that learns a correction to the known physics and is trained to be differentiable, so that it can later serve as the model inside a fuel-optimal control routine.
