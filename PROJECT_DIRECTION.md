...
| **Surrogate architecture** | **GNN graph network simulator** (not transformer, not Neural-ODE) | Message passing over the consist chain graph, advanced by one learned fixed step per stored transition; transformer demoted to an ablation/route-encoder, Neural-ODE demoted to related work. _Revised 2026-09-17 from "GNN / Neural-ODE" — see `SURROGATE_FORMULATION_NOTE.md`._ |
...
> **Superseded 2026-09-17.** The original design on this line was a physics-residual
> (`ŷ = y_phys + Δy_GNN`) integrated as a Neural ODE. It was dropped before any model code
> was written: the residual is identically zero because `data/v2` was generated by the same
> RHS, and the design is slower than the simulator it replaces. Measurements and the full
> argument are in `SURROGATE_FORMULATION_NOTE.md`. The original wording is preserved in git
> history (commit `c9d212b` and earlier). What follows is the current design.

**Design: a message-passing GNN that predicts the mean acceleration of each vehicle over one
fixed 0.25 s step — a graph network simulator, in the "Learning to Simulate" sense.**

- Nodes = vehicles, edges = couplers — exactly the existing `H`/`E` chain graph.
- **Structure, not force correction.** Position comes from exact kinematics and the actuator
  lags from the analytic solution of `ż = (u − z)/τ`; the network predicts acceleration only.
  Three of four state channels are governed by imposed formulas rather than fitted weights.
- **Depth is set by physics, not tuning.** A coupler force wave travels ~170–190 m/s against a
  17 m car pitch, so a disturbance reaches ~3 vehicles (5 at p99) within one step. 4–6 rounds
  of message passing, independent of `N`.
...
Baselines to include (so the architecture claim is earned, not asserted): zero-output
(predict no change — 0.0176 m/s median, the number to beat), constant-acceleration
extrapolation, a per-vehicle MLP with no message passing, and the demoted transformer.
_Note: "physics-only" cannot be a baseline at this step size — one explicit RK4 step at
`h = 0.25` diverges. See `SURROGATE_FORMULATION_NOTE.md`._
...
5. **Surrogate model** — representation, GNN graph network simulator with a learned fixed
   step, baselines, training.
   _(Rewrite of Ch4 around the GNN; transformer and Neural-ODE → ablation / related work.)_
...
**Critical path:** real-terrain route pipeline → batched simulator + dataset → GNN graph
network simulator → surrogate results (Ch6) → MPC control demo (Ch7).
